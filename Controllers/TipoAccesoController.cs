...
namespace APIRestV2.Controllers
{
    ...
    public class TipoAccesoController : ControllerBase
    {
        ...
        private ProcessTipoAcceso process = new();
        ...
        public TipoAccesoController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        public ActionResult Post([FromBody] RequestTipoAcceso req)
        {
            ...
            try
            {
                ...
                if (req.DescTipoAcceso != null)
                {
                    var result = process.AddTipoAcceso(req, remoteIpAddress.ToString());
                    if (result != null)
                    {
                        return Ok(result);
                    }
                    else
                    {
                        ...
                        return NotFound("TipoAcceso not found");
                    }
        ...
        [HttpPut()]
        public ActionResult Put([FromBody] RequestTipoAcceso req)
        {
            var remoteIpAddress = HttpContext.Request.HttpContext.Connection.RemoteIpAddress;
            try
            {

                ResponseGral result = process.UpdateTipoacceso(req, remoteIpAddress.ToString());
                if (result != null)
                {
                    return Ok(result);
                }
        ...
        [HttpGet("{id}")]
        public ActionResult<TipoAcceso> Find(long id) 
        {
            try
            {
                if (id <=0)
        ...
        [HttpGet()]
        public ActionResult<List<TipoAcceso>> FindAll()
        {
            try
            {
               
                List<TipoAcceso> result = process.FindAllTipoAcceso();
                if (result != null)
                    {
                        return result;
                    }
